...
- `type`: The type of question, options include:
    ...
    - `leaflet`: A map type of question using the `leaflet` package.
...
#### Use `type = 'leaflet'` to specify a map type:

::: {.callout-note}

The `leaflet` type of question is always reactive, so it can only be defined in `app.R` and called in `survey.qmd` using the `sd_output()` function.
